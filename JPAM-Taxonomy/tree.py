...
def top_n_words(topic,
    ...
    return [ "%s|%f" % (vocab[i], topic[i]) for i in indices[0:nwords]]
...
def main(type,
    ...
    vocab = map(str.strip, open(vocab_filename, "r").readlines())
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 6:
        docmap = read_docmap("corpus791.map")
        vocab = Dictionary.load("corpus791.dict")
        # state = read_state("skewed_out/run000/mode", vocab, 5)
        # add_assignments_to_tree("skewed_out/run000/mode.assign", state["tree"])
        # write_topic_tree_ascii(state, docmap, "791docs_8330iter_skewed.txt")
        state = read_state("fewtopics_out/run003/mode", vocab, 5)
        add_assignments_to_tree("fewtopics_out/run003/mode.assign", state["tree"])
        write_topic_tree_ascii(state, docmap, "791docs_8648iter_few.txt")
    else:
        type = sys.argv[1]
